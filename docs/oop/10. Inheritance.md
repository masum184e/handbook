...
<summary>Is it possible to inherit child from a non-abstract class and interface?</summary>
...
* **Single inheritance (class → class)**
...
* `Bird` **extends** `Animal` (a non-abstract class ✅)
* `Bird` **implements** `Flyable` (an interface ✅)
...
* `extends` → **"is-a" relationship** (Bird is an Animal)
* `implements` → **"can-do" abilities** (Bird can Fly)
