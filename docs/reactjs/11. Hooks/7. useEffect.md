...
`useEffect` runs after rendering, so it’s safe to perform these operations.
...
- Cleanup function (optional) → runs before unmount or before next effect
...
- Components re-render whenever state changes
- API calls are side effects (they happen outside rendering)

`useEffect` is designed to handle sFFide effects like:
...
You should NEVER fetch data directly inside the component body, because it will run on every render and cause infinite loops.

## `useEffect` vs Lifecycle Methods (Class Components)

| Lifecycle Phase | Class Component      | useEffect Equivalent        |
| --------------- | -------------------- | --------------------------- |
| Mount           | componentDidMount    | useEffect with `[]`         |
| Update          | componentDidUpdate   | useEffect with dependencies |
