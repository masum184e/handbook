...
- `CMD` → default command, but can be overridden:
  ...
  ```bash
  docker run flask-app python other.py
  ```

  → Runs python other.py instead of app.py.
...
- `ENTRYPOINT` → always executed, arguments are appended:
  ...
  - Override:
    ...
    ```bash
    docker run flask-app other.py
    ```

    → Runs python `other.py`.

Best practice: use ENTRYPOINT for main command, CMD for default args.
...
This is widely used in production to keep Docker images small, fast, and secure. Let’s break it down step by step.

### What are Multi-stage Builds?

- Normally, when you build an image, everything you install (compilers, tools, libraries) stays inside the image.
- This makes the image large and includes unnecessary files.
- Multi-stage builds let you use multiple FROM instructions in one Dockerfile.
- You build your app in one stage (with all heavy build tools), then copy only the final output into a smaller runtime image.

Benefit: Smaller, cleaner, faster images.
